...
void main() async {
  ...
  for (int page = 1; page <= 604; page++) {
    ...
    if (textLineCount != dbLineCount) {
      pagesWithMismatch++;
      if (pagesWithMismatch <= 20) {
        mismatchDetails.add("P$page: Text=$textLineCount lines | DB=$dbLineCount lines");
      }
    }
  ...
  if (mismatchDetails.isNotEmpty) {
    print("\nFirst ${mismatchDetails.length} mismatches:");
    for (var detail in mismatchDetails) {
      print("  $detail");
    }
  }
